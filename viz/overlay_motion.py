...
def estimate_attitude(samples):
    ...
    Bias comes only from quiet samples in the first two seconds. Missing samples
    are not integrated across; the exported manifest reports those gaps.
    """
    ...
    for i in range(1, len(times)):
        ...
        if dt > MAX_GAP:
            ...
            result[i] = q
            continue
        omega = (gyro[i - 1] + gyro[i]) * 0.5 - bias
